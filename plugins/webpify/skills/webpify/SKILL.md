...
- [よくある使用パターン](#よくある使用パターン)
  ...
  - [品質を指定して変換](#品質を指定して変換)
  - [再帰的に変換（サブディレクトリ含む）](#再帰的に変換サブディレクトリ含む)
  - [出力先を別ディレクトリに指定](#出力先を別ディレクトリに指定)
  - [既存ファイルを上書きして再変換](#既存ファイルを上書きして再変換)
  - [lossless（可逆圧縮）モードで変換](#lossless可逆圧縮モードで変換)
  - [WebP ファイル一覧を確認](#webp-ファイル一覧を確認)
  - [WebP ファイル一覧を絶対パスで表示](#webp-ファイル一覧を絶対パスで表示)
  - [CI/CD 向け（サイレント実行）](#cicd-向けサイレント実行)
- [品質設定ガイドライン](#品質設定ガイドライン)
- [出力形式](#出力形式)
  - [単一ファイル変換時](#単一ファイル変換時)
  - [ディレクトリ変換時](#ディレクトリ変換時)
  - [--list 実行時](#--list-実行時)
...
- [トラブルシューティング](#トラブルシューティング)
  - [変換後もファイルサイズが大きい](#変換後もファイルサイズが大きい)
  - [既存の WebP がスキップされる](#既存の-webp-がスキップされる)
  - [サブディレクトリの画像が変換されない](#サブディレクトリの画像が変換されない)
- [追加リソース](#追加リソース)
